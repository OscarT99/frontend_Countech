...
export class ProduccionComponent implements OnInit {
  ...
  messageService: any;
    ...
    listProcesos: ProcesoReferenciaPedidoInstance[] = [];

    listAsignarProceso: AsignarProcesoEmpleado[] = []
...
getAsignarProcesoEmpleado() {
  this._AsignarProcesoService.getAsignarProcedimientos().subscribe((data: any) => {
      this.listAsignarProceso = data.Procesos;
...
async actualizarCantidadHecha(procesoId: number): Promise<void> {
  try {
    const AsignarProcesoEmpleado = this.listAsignarProceso
      .filter(asignar => asignar.proceso === procesoId && asignar.estado === true);

    const cantidadHecha = AsignarProcesoEmpleado.reduce((suma, asignar) => suma + (asignar.cantAsignada || 0), 0);
